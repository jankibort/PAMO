...
class CannonView(context: Context?, attrs: AttributeSet?) : SurfaceView(context, attrs), SurfaceHolder.Callback {
    ...
    private val blocker = arrayOfNulls<Blocker>(3)
    ...
    fun newGame() {
        ...
        for (i in blocker.indices) {
            blocker[i] = Blocker(this, Color.BLACK, MISS_PENALTY,
                    (BLOCKER_X_PERCENT * screenWidth).toInt(),
                    ((0.5 - BLOCKER_LENGTH_PERCENT / 2) * screenHeight * i + 1 / 2).toInt(),
    ...
    fun testForCollisions() {
        ...
        if (cannon!!.getCannonball() != null &&
            ...
            while (n < targets!!.size) {
                if (cannon!!.getCannonball()!!.collidesWith(targets!![n])) {
                    ...
                    timeLeft += targets!![n].getHitReward().toDouble()
        ...
        for (i in blocker.indices) {
            if (cannon!!.getCannonball() != null &&
                ...
                timeLeft -= blocker[i]!!.getMissPenalty().toDouble()
            }
        }
    }
    ...
    companion object {
        ...
        const val BLOCKER_LENGTH_PERCENT = 1.0 / 8
